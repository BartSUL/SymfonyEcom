...
class ProductFixtures extends Fixture
{
    public function load(ObjectManager $manager): void
    {
        ...
        //La liste de nos différentes catégories sous forme d'une tableau associatif, contnant une indication du type de catégorie sous le clef et l'objet Category en valeur.
        ...
        foreach($categoryArray as $key => &$value){ //Nous récupérons le tableau des catégories à implémenter
            //Le & avant $value indique une référence, ce qui signifie que nous récupérons la variable en tant que telle plutôt que sa valeur, ce qui nous permet de  modifier notre tableau $categoryArray plutôt qu'une copie de $value qui sera supprimée après la boucle
            ...
            $value->setDescription($lorem); //La description est un lorem impsum générique
        ...
        foreach($productArray as $productData){ //Nous lisons chaque tableau associatif renommé temporairement $productData
            $product = new Product; //On crée un nouveau Produit avant de le renseigner
            $product->setName($productData['name']);
            $product->setDescription($productData['description']);
            $product->setPrice($productData['price']);
            $product->setStock($productData['stock']);
            //Nous récupérons l'objet de categoryArray tenu par la clef dont le nom est fourni par la valeur de "category" de $productData
            $product->setCategory($categoryArray[$productData['category']]);
            $manager->persist($product); //Demande de persistance du Product
        }

        //Nouveaux produits aléatoires (boucle alternative)
        //On crée une liste des catégories potentielles
        $categories = ['chaise', 'bureau', 'lit', 'canape', 'armoire', 'autre'];
        for($i=0; $i < 58; $i++){
            //On sélectionne un nom de catégorie au hasard qui servira à nommer notre Product et à déterminer la clef à sélectionner dans $categoryArray
